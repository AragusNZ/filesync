...
**`filesync config show`** prints the effective system home, repo path anchor, and paths to global JSON files. **`filesync doctor`** (or **`filesync doctor inspect`**) summarizes **`FILESYNC_HOME`** (if set), warns if **`repos.json`** contains duplicate **`name`** values, verifies checkout directories exist, and—when the current directory is inside a filesync project—checks **`files.json`** for duplicate **`local_path`** values and unknown **`repo_id`** references, verifies coupled **`kind=clone`** marker **`path=`** / **`repo=`** / **`repo_id=`** tokens against each row and the global catalog, warns on **`kind=clone`** files with no mapping row, and scans for **`kind=master`** markers whose canonical **`repo_id` + `repo_file_path`** has no tracked rows in any known project’s **`files.json`** (same notion of “clones” as **`filesync info file`**). It prints grouped sections and a short summary of warnings and notes. Use **`filesync doctor clean`** to remove ghost non-master markers on disk.
...
- **`true`**: before the first content-changing sync for that repo in a run, the project must have **no dirty paths** except possibly this project’s **`files.json`** (under your **`FILESYNC_DIR`**, usually **`.filesync/files.json`**)—for example after **`check`** or the embedded **`check`** in **`sync -c`**. Any other change still blocks the git batch. **`sync`** creates a short-lived branch `filesync-sync-…` (no slash, so it does not collide with a branch literally named **`filesync`**), writes updated clones and the refreshed **`files.json`** there, commits them in one step, checks out your previous branch, and **`git merge`**’s the sync branch (then deletes it on success). If **`git checkout -b`** fails with a ref path error, **`sync`** prints a hint to rename or remove the blocking branch. If the batch has **no** content changes but deferred “already in sync” metadata updates, it may **commit** only **`files.json`** on your branch. Conflicting paths may be marked **`conflict`** in **`files.json`**; up to **three** merge attempts can drop conflicting paths and retry the rest. Use normal git conflict resolution if a merge stops mid-way.
...
- **`--file=fragment`**: for **`check`**, **`sync`**, and **`list files`**, substring match on project-relative **`local_path`** only. Repeat **`--file=`** for OR within this dimension.
- **`--repo-file=fragment`**: substring match on **`repo_file_path`** only (repeat for OR within this dimension).
- **`--all-files=fragment`**: substring match on **`local_path` *or* `repo_file_path`** (repeat for OR within this dimension).
- **Combining dimensions**: **`--file`**, **`--repo-file`**, and **`--all-files`** are **AND**ed when more than one dimension has at least one nonempty fragment after trimming. Omit a dimension (or pass only blank values) to leave it unconstrained. Whitespace is trimmed per value. Not valid for **`list repos`** or **`list collections`** (path filters as a group).
- **`list collections`** accepts no options (no **`--repo`**, path filters, **`--status`**, or **`--include-detached`**).
...
**`filesync push --to-clones <path>`** resolves **`path`** like **`info file`** (tracked clone or master under a repo checkout), then runs **`sync -c --repo=… --file=… -f --status=all`** in each project (one **`--file=`** per affected **`local_path`** in that project) that has a related row so the edited master overwrites every clone (multi-project union). **`sync -c`** refreshes **`sync_status`** first; **`--status=all`** ensures each selected row is still diffed and updated if the working-tree master and local clone differ. **`--dry-run`** is passed through to **`sync`**. Do not combine **`--to-clones`** with **`--all`** or extra **`local_path`** arguments. Full behavior is in **`man filesync`**.
