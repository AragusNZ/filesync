...
prefix_smoke() {
	...
	(
		cd "${TMP}/proj-${label}"
		export FILESYNC_HOME="${TMP}/fsys-${label}"
		PATH="${dest}${prefix}/bin:${PATH}" filesync init
		[[ -f .filesync/files.json ]] || die "init missing files.json (${label})"
		[[ -f "${FILESYNC_HOME}/repos.json" && -f "${FILESYNC_HOME}/collections.json" && -f "${FILESYNC_HOME}/system.json" ]] || die "init missing system store (${label})"
