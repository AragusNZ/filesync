...
_hcd 'check (c)' 'Verify mappings; refresh row status' '[--repo=name] [--file=path_fragment | --exact-local=path …] [--status=a,b,…]'
...
_hcd 'retarget' 'After git mv: point repo_file_path at the new master (run on a clone OR the master file; scope differs)' '<local_file|old_path> <new_repo_file_path> [--move|--mv]  (clone: one row; master: all sibling projects; see retarget -h)'
_hcd 'sync (s)' 'Copy from masters into the project; update files.json' '[--repo=name] [--file=path_fragment | --exact-local=path …] [-c|--check] [--dry-run] [-f|--force] [--showall] [--status=a,b,…] [--include-detached] [--move|--mv]'
...
echo ''
echo '   merge_using_git (per repo in global repos.json): when true and the project is a clean git'
echo '   work tree, sync applies changes via a short-lived branch and merge; see docs/configuration.md.'
echo ''
echo 'Config:'
