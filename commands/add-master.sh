...
for i in "${!LOCAL_PATHS[@]}"; do
  ...
  if ! render_master_marker_file "$full_local_path" "$TMP_MASTER"; then
    cp "$full_local_path" "$TMP_MASTER"
    if ! prepend_master_marker_to_file "$TMP_MASTER" "$full_local_path"; then
      echo -e "${RED}Error: Could not add a filesync marker to local file.${NC}" >&2
      exit 1
    fi
  fi
