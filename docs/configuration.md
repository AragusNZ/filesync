...
- **`true`**: before the first content-changing sync for that repo in a run, the project must have **no dirty paths** except possibly this project’s **`files.json`** (under your **`FILESYNC_DIR`**, usually **`.filesync/files.json`**)—for example after **`check`** or the embedded **`check`** in **`sync -c`**. Any other change still blocks the git batch. **`sync`** creates a branch `filesync/sync-…`, writes updated clones there, commits, checks out your previous branch, and **`git merge`**’s the sync branch (then deletes it on success). Conflicting paths may be marked **`conflict`** in **`files.json`**; up to **three** merge attempts can drop conflicting paths and retry the rest. Use normal git conflict resolution if a merge stops mid-way.
