...
**`info`** (alias **`i`**) takes an optional **`file`** or **`-f`** word then a **`local-path`**, or **`filesync info <local-path>`** / **`i <local-path>`** with no **`file`** / **`-f`** keyword. It refreshes every mapping that shares the same master across known projects (**`check`** with **`--exact-local=`**), prints a summary, then optionally prompts (or **`--fix-marker`**) to add or remove **`kind=master`** on the canonical master file when it does not match whether clones exist. See **`filesync info file --help`** (or **`filesync i --help`**) and **`man filesync`**.
