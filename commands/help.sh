...
_hcd 'enable' 'Set check_sync_enabled true for a repo (global store)' '<repo_name>'
_hcd 'disable' 'Set check_sync_enabled false for a repo (global store)' '<repo_name>'
_hcd 'progress' 'TTY progress (writes preferences.json in system store)' '[hidden|bar|percent]'
_hcd 'config' 'Show/doctor/set system home, preferences, repo flags (repo path anchor is home)' 'show | doctor | set ... | repo ...'
...
_hcd 'handle-missing' 'Repair rows when master or local paths are missing (non-interactive flags)' 'Project cwd; --local-path= + one action; see filesync handle-missing -h'
echo ''
...
_hcd 'list-collections (lcol)' 'List repo collections (--also=)' ''
echo ''
echo 'Mappings and repos:'
_hcd 'add-file (af)' 'Track from repo checkout; path_in_repo is under the repo; omit :local if same path' '<repo> <path_in_repo>[:<local_path>] … [--mark-master] [--also=repos_or_collections]'
_hcd 'add-master (am)' 'Promote local file to master; omit :path_in_repo if it matches local_path' '<repo> <local_path>[:<path_in_repo>] … [--also=repos_or_collections]'
_hcd 'add-clone (ac)' 'Clone from kind=master in this project into sibling(s); fails if target exists' '<target_repo> <master_path>[:<local_path>] … [--also=repos_or_collections]'
