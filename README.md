...
**`sync`** / **`list-files`** status filter **`--status=a,b,...`** (see [docs/configuration.md](docs/configuration.md)): same tokens for both; **`all`** is every status except **`detached`** unless **`--include-detached`** or **`detached`** is listed; **`error`** matches any **`error_*`**; **`unset`** is empty status. **`sync`** default (no **`--status`**) is **`unset`** + **`sync_required`**; add **`--include-detached`** to allow **`detached`** there too. **`sync`** also supports **`--dry-run`**, **`--force`** (overwrite locals that lack the clone marker), **`--showall`** (print per-file lines for files already in sync; default hides them).
