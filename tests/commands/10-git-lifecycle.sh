...
(
	...
	jq -e '.[] | select(.local_path=="tools/demo.txt") | .repo_id == "testid-origin" and (has("repo_name") | not)' ".filesync/files.json" >/dev/null \
		|| die "edit-repo rename: files.json keeps stable repo_id, no persisted repo_name"
